 ...
 *  - "MenuItems"  : Global item registry (all items ever created)
 *  - "MelaMenu"   : Per-mela menu config (which items, at what price)
 *  - "mela_{id}"  : One purchases sheet per mela (auto-created)
 *
 * PURCHASES FORMAT:
 *  - New sheets (Phase 3+): 9 columns, Items stored as JSON
 *  - Old sheets (pre-Phase 3): 17 columns, one column per item
...
const MELAS_HEADERS = [
  'MelaId','MelaName','Area','OrganiserName',
  'ContactPerson','ContactPhone','DateFrom','DateTo','Status','CreatedAt',
];
...
// New purchases format (Phase 3+) — Items stored as JSON string
const PURCHASES_HEADERS_NEW = [
  'Timestamp','Date','CustomerName','Phone',
  'CustomerType','SocialMedia','Items','TotalAmount','PaymentMode',
];

// Old column → item key (backward compat for pre-Phase 3 sheets)
...
// Default items — seeded into MenuItems on first run
const DEFAULT_ITEMS = [
...
function getMenuItemsSheet(ss) {
  ...
  if (!s) {
    ...
    s.appendRow(MENU_ITEMS_HEADERS);
    styleHeader(s, MENU_ITEMS_HEADERS.length);
...
/* ═══════════════════════════════════════
   MELA HANDLERS
═══════════════════════════════════════ */
function handleGetMelas(ss) { return ok({ data: sheetToObjects(getMelasSheet(ss)) }); }

function handleCreateMela(ss, e) {
  const d = JSON.parse(e.parameter.data);
  const melaId = 'm_' + Date.now();
  getPurchasesSheet(ss, melaId);
  getMelasSheet(ss).appendRow([
    melaId, d.name||'', d.area||'', d.organiser||'',
    d.contactPerson||'', d.contactPhone||'',
    d.dateFrom||'', d.dateTo||'', 'active', new Date().toISOString(),
  ]);
...
function handleUpdateMela(ss, e) {
  const d = JSON.parse(e.parameter.data);
  const sheet = getMelasSheet(ss);
  const rows  = sheet.getDataRange().getValues();
  const hdrs  = rows[0];
  for (let i=1; i<rows.length; i++) {
    if (rows[i][hdrs.indexOf('MelaId')] === d.melaId) {
      const r   = i+1;
      const set = (col,val) => sheet.getRange(r, hdrs.indexOf(col)+1).setValue(val);
      set('MelaName',d.name||''); set('Area',d.area||'');
      set('OrganiserName',d.organiser||''); set('ContactPerson',d.contactPerson||'');
      set('ContactPhone',d.contactPhone||''); set('DateFrom',d.dateFrom||''); set('DateTo',d.dateTo||'');
...
/* ═══════════════════════════════════════
   MENU HANDLERS
═══════════════════════════════════════ */
function handleGetMenuItems(ss) { return ok({ data: sheetToObjects(getMenuItemsSheet(ss)) }); }
...
function handleCreateMenuItem(ss, e) {
  ...
  const existing = sheetToObjects(mmSheet).filter(r => r.MelaId === d.melaId);
...
function handleUpdateMelaMenuItem(ss, e) {
  ...
  const mmSheet = getMelaMenuSheet(ss);
  const mmRows  = mmSheet.getDataRange().getValues();
  const mmHdrs  = mmRows[0];
  for (let i=1; i<mmRows.length; i++) {
    if (mmRows[i][mmHdrs.indexOf('MelaId')]===d.melaId && mmRows[i][mmHdrs.indexOf('ItemKey')]===d.itemKey) {
      mmSheet.getRange(i+1, mmHdrs.indexOf('SellingPrice')+1).setValue(d.sellingPrice);
      break;
    }
  }
  // Update name in MenuItems — allowed for any item, no mela restriction
  if (d.itemName) {
    ...
    for (let i=1; i<miRows.length; i++) {
      if (miRows[i][miHdrs.indexOf('ItemKey')]===d.itemKey) {
        miSheet.getRange(i+1, miHdrs.indexOf('ItemName')+1).setValue(d.itemName);
        break;
      }
    }
  }
...
function handleSetMenuStatus(ss, e, status) {
  ...
  for (let i=1; i<rows.length; i++) {
    if (rows[i][hdrs.indexOf('MelaId')]===e.parameter.melaId &&
        rows[i][hdrs.indexOf('ItemKey')]===e.parameter.itemKey) {
      sheet.getRange(i+1, hdrs.indexOf('Status')+1).setValue(status);
      return ok({ success:true });
    }
  }
  return ok({ success:false, error:'Item not found in mela menu' });
}

function handleReorderMelaMenu(ss, e) {
  // d = { melaId, order: ['itemKey1', 'itemKey2', ...] }
  const d     = JSON.parse(e.parameter.data);
  const sheet = getMelaMenuSheet(ss);
  let rows    = sheet.getDataRange().getValues();
  let hdrs    = rows[0];

  // Add SortOrder column if it doesn't exist yet (backward compat)
  let sortColIdx = hdrs.indexOf('SortOrder');
  if (sortColIdx === -1) {
    const newCol = hdrs.length + 1;
    sheet.getRange(1, newCol).setValue('SortOrder');
  ...
  const melaIdCol  = hdrs.indexOf('MelaId');
  const itemKeyCol = hdrs.indexOf('ItemKey');
  const orderMap   = {};
...
function handleSavePurchase(ss, e) {
  ...
  if (isNewFormatSheet(sheet)) {
    ...
    // Old format — extract qty from items (which may be {qty,price} or plain number)
